...
void* HeapAllocator::kmalloc(size_t size) {
    ...
    while (current != nullptr) {
        ...
        // Split block if remaining space can fit another header + min payload
        ...
        current->free = false;
        m_total_used += current->size;
        m_total_free -= current->size;
...
void HeapAllocator::kfree(void* ptr) {
    ...
    if (free_block->free) return; // Prevent double-free

    free_block->free = true;
    m_total_used -= free_block->size;
    m_total_free += free_block->size;

    // Coalesce forward (merge with next block if free)
    if (free_block->next != nullptr && free_block->next->free) {
        free_block->size += sizeof(Block) + free_block->next->size;
        free_block->next = free_block->next->next;
        m_block_count--;
    }

    // Coalesce backward (merge with previous block if free)
    Block* previous = m_head;
    if (free_block != m_head) {
        while (previous != nullptr && previous->next != free_block) {
...
void* kmalloc(size_t size) {
    return HeapAllocator::kmalloc(size);
}
